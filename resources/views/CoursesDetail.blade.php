...
<div class="w-full px-6 py-4">
  ...
  <div class="max-w-screen-xl mx-auto lg:flex lg:items-center lg:min-h-[300px]">
    ...
    <div class="flex-1 text-left space-y-4 text-base lg:text-[1rem]">
      ...
      @auth
        @if ($isEnrolled)
          <span class="inline-block bg-gray-400 text-white font-bold text-base px-6 py-2 rounded-md cursor-not-allowed opacity-75">
            ✓ Already Enrolled
          </span>
        @else
          <form action="{{ route('courses.enroll', $course->id) }}" method="POST" class="inline-block">
            @csrf
            <button type="submit"
                    class="bg-[#8A60A2] text-white font-bold text-base px-6 py-2 rounded-md hover:bg-opacity-90 transition">
              Enroll Now
            </button>
          </form>
        @endif
  ...
  <!-- Gambar -->
  <div class="mt-6">
    <img src="{{ asset($course->image_path_1) }}" alt="Course visual" class="rounded-lg w-full object-cover" />
  </div>
